...
mzRComponent2 <- function(
  ...
  out <- list(
    ...
    sdPrev = sde2,
    cvPrev = sde2 / est2,
    cilPrev = quantNA(estb2, .025),
    ciuPrev = quantNA(estb2, .975),
    absChange = absdiff,
    sdAbsChange = sdabs,
    cvAbsChange = sdabs / absdiff,
    cilAbsChange = quantNA(absdiffb, .025),
    ciuAbsChange = quantNA(absdiffb, .975),
    relChange = ratediff,
    sdRelChange = sdrel,
    cvRelChange = sdrel / ratediff,
    cilRelChange = quantNA(ratediffb, .025),
...
CompFehlerX <- function(
  ...
  if (is.null(each)) {
    ...
    if (any(grepl("Prev", names(x)))) {
      ...
      row.names(print_obj) <- c(
        x[["date"]],
        x[["datePrev"]],
        "Absolute change",
        "Relative change"
      )
      colnames(print_obj)[1:5] <- c(
        "est",
        "sd",
        "cv",
        paste(
          names(x[c("cil", "ciu")]),
          unlist(lapply(x[c("cil", "ciu")], names)),
          sep = "_"
        )
      )
    } else {
    ...
    if (any(grepl("Prev", names(x[[1]])))) {
      ...
      for (i in 1:length(x)) {
        ...
        colnames(print_obj)[1:5] <- c(
          ...
          paste(
            names(x[[i]][c("cil", "ciu")]),
            unlist(lapply(x[[i]][c("cil", "ciu")], names)),
            sep = "_"
          )
        )
...
as.data.frame.mzR <- function(x, ...) {
  ...
  for (i in 1:length(id)) {
    vals[[i]] <- substring(
      id[i],
      rev(pmax(seq(nchar(id[i]) - 2, -1, -3))),
      rev(seq(nchar(id[i]), 1, -3))
    )
  }
